...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query(value = "SELECT u.id AS id, u.email AS email, u.username AS username, u.firstname AS firstname, u.lastname AS lastname " +
                   "FROM user u " +
                   "WHERE id IN (SELECT c.contact_user_id c FROM contacts c WHERE c.user_id = :userId) " +
                   "ORDER BY u.id " +
